...
class LinearModelTest(TestCase):
    ...
    @parameterized.expand(
        [
            ...
            [
                "godiegxy",
                TEST_DATA["godiegxy"]["ts"],
                TEST_DATA["godiegxy"]["freq"],
                TEST_DATA["godiegxy"]["res_95"],
                TEST_DATA["godiegxy"]["res_99"],
            ],
            [
                "invalid",
                TEST_DATA["godiegxy"]["ts_nan"],
                TEST_DATA["godiegxy"]["freq"],
                TEST_DATA["godiegxy"]["res_invalid"],
                TEST_DATA["godiegxy"]["res_invalid"],
            ],
        ]
    )
    ...
    @parameterized.expand(
        [
            [
                TEST_DATA["godiegxy"]["ts"],
                TEST_DATA["godiegxy"]["invalid_param_1"],
                TEST_DATA["godiegxy"]["freq"],
                TEST_DATA["godiegxy"]["res_invalid_param_1"],
            ],
            [
                TEST_DATA["godiegxy"]["ts"],
                TEST_DATA["godiegxy"]["invalid_param_2"],
                TEST_DATA["godiegxy"]["freq"],
                TEST_DATA["godiegxy"]["res_invalid_param_2"],
            ],
        ]
    )
    ...
    def test_exec_plot(self) -> None:
        ...
        # Fit forecast
        m = LinearModel(cast(TimeSeriesData, TEST_DATA["godiegxy"]["ts"]), params)
        m.fit()
        _ = m.predict(steps=2, freq=TEST_DATA["godiegxy"]["freq"])
        # Test plotting
        m.plot()

    def test_name(self) -> None:
        m = LinearModel(
            cast(TimeSeriesData, TEST_DATA["godiegxy"]["ts"]), LinearModelParams()
        )
